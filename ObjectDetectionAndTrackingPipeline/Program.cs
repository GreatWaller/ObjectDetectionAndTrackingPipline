...
namespace ObjectDetectionAndTrackingPipeline
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            ModuleRegistration.RegisterAll();
            ...
            foreach (var config in settings.Pipelines)
            {
                ...
                if (config.VideoCaptureModule.Type.Contains("Skia"))
                {
                    pipeline = PipelineFactory.CreatePipeline<SKBitmap>(config, cts.Token);
                }
                else
                {
                    pipeline = PipelineFactory.CreatePipeline<Mat>(config, cts.Token);
                }
